...
test.describe("Basic rendering", () => {
  ...
  test("should render toolbar with create button", async ({ page }) => {
    await expect(page.locator(".table-edit-toolbar")).toBeVisible();
    await expect(page.locator(".table-edit-toolbar .toolbar-button")).toBeVisible();
    await expect(page.locator(".table-edit-toolbar .toolbar-input")).toBeVisible();
  });
...
test.describe("Cursor navigation", () => {
  test("should navigate with arrow keys", async ({ page }) => {
    const table = page.locator(".table-edit");
    await table.focus();
  ...
  test("should navigate with PageDown to last row", async ({ page }) => {
    ...
    const viewport = page.locator(".table-edit-viewport");
...
test.describe("Row creation", () => {
  test("should create new rows using toolbar", async ({ page }) => {
    ...
    // Set count to 3
    const input = page.locator(".table-edit-toolbar .toolbar-input");
    await input.fill("3");

    // Click create
    await page.locator(".table-edit-toolbar .toolbar-button").click();
...
test.describe("Context menu", () => {
  ...
  test("should not open context menu when editor dialog is open", async ({ page }) => {
    ...
    await page.locator(".table-edit-viewport").click({ button: "right", position: { x: 10, y: 10 }, force: true });
    await page.waitForTimeout(100);
  ...
  test("should show undo/redo in context menu with correct disabled state", async ({ page }) => {
    ...
    await page.locator(".table-edit").click();
    await page.waitForTimeout(100);
    await cell.click();
    await page.keyboard.press("Delete");
    await page.waitForTimeout(200);
...
test.describe("Selection rectangle", () => {
  ...
  test("should position selection rectangle over a non-sticky cell after click", async ({
    ...
    const rect = await page.evaluate(() => {
      const viewport = document.querySelector(".table-edit-viewport") as HTMLElement;
...
test.describe("Backend mode – error handling (server error)", () => {
  ...
  test("rollback triggers shake animation on the table", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const el = document.querySelector(".table-edit");
      if (!el) return;
...
test.describe("Empty area below rows", () => {
  test("table should not stretch rows when few rows are displayed", async ({ page }) => {
    ...
    const viewport = page.locator(".table-edit-viewport");
    const table = page.locator(".table-edit-viewport table");
